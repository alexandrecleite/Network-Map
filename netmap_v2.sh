...
# 2.0
# modded by Alexandre C. Leite to add convert pcap to csv
...
# Generate CSV file from pcap by ACL
if [ $# -eq 0 ]; then
    echo "Use $0 pcapfile.pcap"
	exit
else tshark -nr $1 -qz conv,ip | tail +6 | head -n -1 > tmp.csv
	awk '{print $1 "," $3}' tmp.csv | sed 's/^/"/g' | sed 's/,/","/g' | sed 's/$/"/g' > conversation.csv
	rm tmp.csv
fi
